...
export async function GET(request: Request) {
  ...
  if (!code) {
    return NextResponse.redirect('https://2aaa0f3492c3.ngrok-free.app/dashboard?error=MissingCode');
  }

  if (!sessionToken) {
    return NextResponse.redirect('https://2aaa0f3492c3.ngrok-free.app/dashboard?error=MissingSession');
  }

  if (!codeVerifier) {
    return NextResponse.redirect('https://2aaa0f3492c3.ngrok-free.app/dashboard?error=MissingVerifier');
  }
  ...
  try {
    ...
    if (existingMLUser && existingMLUser.id !== userId) {
      ...
      return NextResponse.redirect('https://2aaa0f3492c3.ngrok-free.app/dashboard?error=MLAccountAlreadyLinked');
    }
    ...
    const response = NextResponse.redirect('https://2aaa0f3492c3.ngrok-free.app/dashboard?success=true');
    ...
    return NextResponse.redirect('https://2aaa0f3492c3.ngrok-free.app/dashboard?error=TokenError');
  } finally {
